...
/* ===============================
   TEST ORDER FOR admin@cinema.com
================================ */
...
IF @AdminUserId IS NOT NULL
    BEGIN
        ...
        -- Create Order
        DECLARE @OrderId INT;
        INSERT INTO Orders (UserId, SessionId, Status, CreatedAt)
        VALUES (@AdminUserId, @AdminSessionId, 1, GETDATE());
        SET @OrderId = SCOPE_IDENTITY();

        /* ===============================
           SEAT RESERVATIONS + TICKETS
        ================================ */

        DECLARE @AdminBasePrice DECIMAL(18, 2);
        SELECT @AdminBasePrice = BasePrice
        FROM Sessions
        WHERE Id = @AdminSessionId;

        DECLARE @AdminSeatId INT;
        DECLARE @AdminSeatPrice DECIMAL(18, 2);
        DECLARE @SeatReservationId INT;

        DECLARE admin_seat_cursor CURSOR FOR
            SELECT TOP 3 s.Id, (@AdminBasePrice + st.AddedPrice)
            FROM Seats s
                     JOIN SeatTypes st ON s.SeatTypeId = st.Id
                     LEFT JOIN SeatReservations sr
                               ON sr.SeatId = s.Id AND sr.SessionId = @AdminSessionId
            WHERE s.HallId = 1
              AND sr.Id IS NULL -- тільки вільні місця
            ORDER BY s.RowNum, s.SeatNum;

        OPEN admin_seat_cursor;
        FETCH NEXT FROM admin_seat_cursor INTO @AdminSeatId, @AdminSeatPrice;
        ...
        WHILE @@FETCH_STATUS = 0
            BEGIN
                ...
                INSERT INTO Tickets (OrderId, SeatReservationId, Price)
                VALUES (@OrderId, @SeatReservationId, @AdminSeatPrice);

                FETCH NEXT FROM admin_seat_cursor INTO @AdminSeatId, @AdminSeatPrice;
            END

        CLOSE admin_seat_cursor;
        DEALLOCATE admin_seat_cursor;

        /* ===============================
           PAYMENT
        ================================ */
        DECLARE @TotalAmount DECIMAL(18, 2);
        SELECT @TotalAmount = SUM(sr.Price)
        FROM SeatReservations sr
                 JOIN Tickets t ON t.SeatReservationId = sr.Id
        WHERE t.OrderId = @OrderId;

        INSERT INTO Payments (OrderId, Amount, PaymentMethod, PaidAt)
        VALUES (@OrderId, @TotalAmount, 0, GETDATE());

        PRINT 'Admin order created successfully!';
    END
ELSE
    BEGIN
        PRINT 'Warning: Admin user (admin@cinema.com) not found. Skipping admin order creation.';
    END
